	...
	extends JComponent
{
	...
	protected void paintComponent( final Graphics g1 )
	{
		...
		try
		{
			...
			for ( final AbstractBoardSquare boardSquare : board.getBoardSquareList() )
			{
				int xPawns;
				int yPawns;
				...
				if ( boardSquare.getType() == Type.PRISAO )
				{
					final PrisionBoardSquare prision = ( PrisionBoardSquare ) boardSquare;
					final int initXPrision = prision.getInitialXPositionJail();
					final int initYPrision = prision.getInitialYPositionJail();
					final int finalXPrision = prision.getFinalXPositionJail();
					final int finalYPrision = prision.getFinalYPositionJail();

					final int xLengthPrision = finalXPrision - initXPrision;
					final int yLengthPrision = finalYPrision - initYPrision;

					final int xDiffPrision = xLengthPrision / xPawns;
					final int yDiffPrision = yLengthPrision / yPawns;
					int xPrision = initXPrision;
					int yPrision = initYPrision;
					count = 1;

					for ( final Pawn pawn : prision.getPawnListOnPrision() )
					{
						g.setColor( pawn.getColor() );
						g.fillOval( xPrision + 2, yPrision + 4, 25, 25 );

						count++;
						if ( count <= xPawns )
						{
							xPrision += xDiffPrision;
						}
						else
						{
							count = 1;
							xPrision = initXPrision;
							yPrision += yDiffPrision;
						}
					}
				}
